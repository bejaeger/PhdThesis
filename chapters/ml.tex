...
\Cref{sec:intro-neural-nets} describes the structure of neural networks, before \cref{sec:nn-training} explains the learning procedure to which the networks are subjected. \Cref{sec:hyperpar-opt} provides details on how a neural network model is optimized and selected in practice.
...
\begin{figure}[t]
    ...
    \label{fig:simple-nn}
\end{figure}
\TDinote{}{Add colors around gradients according to figure}
...
\TDinote{}{Maybe put the performance metric section in a separate section}
...
\begin{itemize}
    ...
    \item Test set: the subset used for final tests of generalization and to deploy the model. 
\end{itemize}
...
Here, the dataset used for the training is usually required for further statistical interpretations. In such cases, it is best practice to use the test set for further analysis as it removes any potential bias that may occur when data samples are evaluated with a model that was trained with the same data sample.
...
\TDinote{}{Change to nicer colors}
