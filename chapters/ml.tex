...
\TDinote{}{Find references for mentioned ML applications!}
\todo{Mention Higgs analysis and that we use ML for binary classification using supervised learning!!}
...
\todo{First add fully general formula and then use the example with 1 hidden layer.}
...
\todo{Maybe rephrase target values?}
...
\todo{In the above, don't blah blah about performance metrics just yet. Add that as a separate section somewhere. }
...
For HEP applications, other metrics are more suitable. In the case of a physics search, the discovery significance of the signal process over the background process is a more fitting measure as it directly determines the sensitivity and thus performance of an analysis. 
...
For binary classification, the cross entropy loss is commonly chosen. 
...
There are many variations of the gradient descent algorithm that can improve the convergence of the training. An overview of different methods is for example given in \ccite{ruder2017overview}. The \emph{AdaGrad}~\cite{adagrad-duchi} algorithm is one such example that is used in the work presented in this thesis. It adapts the learning rate individually for each parameter by introducing a matrix, $\pmb{G}_{\tau} = \sum_{t=0}^{\tau} \left( \grad_{\pmb{w}^{(t)}} L(\pmb{w}^{(t)}) \right)^2$, that holds the sum of squares of the past gradients for each parameter. 
