...
\end{equation}
%where $\pmb{w}^{(l)}_0$ is the vector holding the bias weights. 
...
% \begin{equation}
%     y(\pmb{x}, \pmb{W}) = \sigma \left( \sum_{h=1}^H w^{(2)}_{kh}h^{(1)}\left(  \sum_{n=1}^{N} w^{(1)}_{hn}x_n + w^{(1)}_{h0}    \right) + w^{(2)}_{0} \right),
% \end{equation}
