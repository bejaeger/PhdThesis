...
The distribution is shown in \cref{fig:random-cones-balance-b} for random cones placed within the central region of the detector (\absetaST{0.7}). The width of the distribution, labelled as $\sigmaRC$, is estimated by taking the \SI{68}{\percent} confidence interval, due to the non-Gaussian behavior.
...
%The width is a measure of the energy fluctuations to which a jet with $R = \Rrandomcone$ is subjected.
...
\Cref{fig:const-scale-noise-results} shows this constituent scale noise as a function of \abseta as well as $mu$ for both, data and MC.
