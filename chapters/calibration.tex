...
\begin{figure}[t]
    \resizebox{\textwidth}{!}{
        \includegraphics[trim=0 20 0 0]{figures/calibration/jes-calibration.pdf}
    }
...
This correction subtracts on average roughly 1-10 GeV from the energy of the original jet, depending on the jet collection considered as well as the pile-up conditions.
...
The exact procedure, referred to as \emph{numerical inversion}, is outlined in \ccite{PERF-2011-03}. The resulting average response is shown in \cref{fig:jes-calibration-jet-response} for different bins of \etadet. 
The response generally decreases with increasing \etadet because of the lower granularity of the calorimeter at high \etadet and the resulting increased calorimeter cluster reconstruction thresholds, which leads to more undetected energy. 
A similar procedure is used to correct biases in the reconstructed jet $\etadet$, which is detailed in \ccite{JETM-2018-05}.
...
This calibration, known as \emph{\insitu} calibration, is derived by comparing the jet response in data and MC simulation in event topologies that allow measuring the jet response with respect to a well-calibrated reference object.
...
The MPF method is complemented by the so-called \emph{multijet balance} method that allows calibrating high-\pT jets ($\pT > 800\,$GeV) by comparing a system of well-calibrated low-\pT jets with a single high-\pT jet~\cite{PERF-2014-02} using multijet events.
...
The following double ratio is applied as the final \insitu correction factor
\begin{equation}
    c = \frac{R_{\insitu}^{\text{data}}}{R_{\insitu}^{\text{MC}}} = \frac{ \langle  \pTcalibData / \pTrefData \rangle }{ \langle \pTcalibMC / \pTrefMC \rangle}.
\end{equation}
The average response, $R_{\insitu}$, is taken from the peak location found by fitting the distribution of the ratio of the \pT of the calibrated probe jet, \pTcalib, and the well-measured reference object, \pTref, with a Gaussian function.
...
\begin{itemize}
    ...
    % \item Sampling fluctuations: fluctuations in the number of ionizing particles crossing the active layers of the sampling calorimeter.
    ...
    \item Fluctuations in the number of ionizing particles crossing the active layers of the sampling calorimeter.
    ...
    \item Fluctuations in the number of heavily ionizing particles, such as alpha particles.
\end{itemize}
...
\begin{figure}[t]
    \newImageResizeCustom{0.85}{figures/calibration/flow-chart-jer.pdf}
...
\begin{equation}
    \Nfull = \sqrt{ \left(\Npileup \right)^2 +  \left( \Nmuzero \right)^2 },
\end{equation}
...
\begin{figure}[t]
    \subfloat[] {
        \newImageResizeCustom{0.46}{figures/calibration/zero-bias-event-deposits-orig.pdf}
    ...
    \subfloat{
        \hspace{-32.13em}    
        \includegraphics[width=0.00153\textwidth,valign=t]{figures/calibration/worm.png}
    }
...
%     \vspace{-500em}
%     \hspace{-10em}
%     \newImageResizeCustom{0.1}{figures/calibration/worm.png}
% \end{figure*}
...
\begin{figure}[h!]
    \newImageResizeCustom{0.53}{figures/calibration/noise-term-results-pflow.pdf}
