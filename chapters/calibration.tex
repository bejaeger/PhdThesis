...
For \RunTwo, the ATLAS collaboration uses a combination of two types of measurements to determine the JER in data. The measurement involves several steps that are summarized in the flow chart shown in \cref{fig:flow-chart-jer} and explained in this section.
...
All dijet MC samples are simulated with \PYTHIA 8.230~\cite{Sjostrand:2007gs} using the {\textsc NNPDF2.3LO}~\cite{Ball:2012cx} set of PDFs and the A3~\cite{ATL-PHYS-PUB-2014-021} tune. The difference between the samples used for the noise term measurement lies in the amount of pile-up that is overlaid and the event selection, that is, whether or not a hard scatter event must be present.
...
The distribution is shown in \cref{fig:random-cones-balance-b} for random cones with radius $\Rrandomcone = 0.4$ within the central region of the detector (\absetaST{0.7}). The width of the distribution, labelled as $\sigmaRC$, is estimated by taking the \SI{68}{\percent} \emph{confidence interval} (CI), due to the non-Gaussian behavior\footnote{The non-Gaussian behaviour is expected to be predominantly caused by topo-clustering threshold effects, that can lead to outliers in the $\Delta \pTrandomcone$ observable.}.
...
\paragraph{A closure test in MC} can be performed for the random cones method by comparing the Response JER extracted from two dijet MC samples, one with pile-up and one without pile-up. The quadratic difference of the Response JER measurements is expected to be dominated by contributions from pile-up. \Cref{fig:non-closure} shows a comparison of the Reponse JER extracted in the different samples and their quadratic difference, as well as the pile-up noise derived with the random cones method in MC simulation. The quadratic difference compared to the noise from MC provides an estimate of the closure of the random cones method.
...
The results are shown in \cref{tab:prelim-jer-2018-noise-term-inputs}.
Uncertainties on this extraction are significant and are described in the following.
...
The uncertainties are propagated to \Nfull, by varying them separately and observing the changes in \Nfull. The propagated uncertainties on \Nfull are then symmetrized and used in the JER combination. Additional uncertainties on \Npileup and \Nmuzero were tested and found to be negligible. These include variations of fit ranges as well as the errors on the parameters from the fits itself. Also, statistical uncertainties are found to be negligible.
...
The resulting JER for PFlow jets is shown in \cref{fig:jer-combination-incl-noise-term-a}. The effect of the noise term is illustrated by displaying the nominally measured value $N / \pT$ and its uncertainties.
The JER taken from MC simulation can be compared to the combined \insitu measurement.
The latter shows significantly smaller uncertainties than the results of either the dijet balance method or the noise term measurement separately, which shows the benefit of the combination procedure.
...
The final JER uncertainties are shown in \cref{fig:jer-combination-incl-noise-term-b}.
The uncertainties from the noise term measurement are propagated to the final results by varying the value of $N$ for each uncertainty source by one standard deviation and repeating the fit.
%The varied $N$ corresponds to the result that is obtained from the noise term measurement when the respective uncertainty is varied by one standard deviation.
...
This is also the case for other $\eta$ regions below $\etadet < 2.5$.
