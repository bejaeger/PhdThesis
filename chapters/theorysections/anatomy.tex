...
\todo{Maybe the first two sentences should go in the general introduction of this chapter.}

The theoretical framework described in \cref{sec:sm} allows making predictions about the probabilities of various interactions between elementary particles. 
These predictions can be tested by studying the outcomes of particle collisions produced by accelerators such as the LHC. 

In order to reach the highest energies, the LHC accelerates proton beams. \todo{Justification for why protons achieve highest energies}
...
This section details the relevant aspects of moving from QCD calculations to observables that can be measured in collider experiments such as the ATLAS detector. 
\Cref{subsec:pp-collision-overview} gives a brief overview of the anatomy of a $pp$ collision event and introduces key terminology. \Cref{subsec:collision-rates} outlines basic observables and \cref{subsec:factorisation} discusses the concepts necessary to compute theoretical particle interaction cross sections.
...
As illustrated in \cref{fig:ppcol}, a hard $pp$ scattering process is characterised by a parton-parton interaction with large momentum transfer, known as \emph{hard scatter}, and further low energy processes.
...
The final-state partons then undergo a process called \emph{hadronisation} (also \emph{fragmentation}), due to the confining nature of QCD. They end up forming a spray of colour-neutral hadrons, which are known as particle \emph{jets}.

\todo{Introduce the word showering here?}
\begin{figure}[h]
...
\begin{figure}
  \newImageResizeCustom{0.65}{figures/anatomy/alphas.png}
...
This behavior is also known as \emph{asymptotic freedom} and implies that strongly interacting particles behave like free particles at high energies. Conversely, at small energies, the particles have a strong coupling, which leads to \emph{confinement} of quarks inside hadrons.
...
Given these considerations, the cross section of a hard-scattering process between two protons, $p_1$ and $p_2$, can be written as
\begin{equation}
...
% Hence, these non-perturbative soft processes are separated from the hard partonic scattering cross-section, a method that is also known as the \emph{factorization theorem}.
...
If the highest order term is linear, that is $m=0$, the calculation is called \emph{leading order} (LO). If $m=1$ ($m=2$), the calculation is called \emph{next-to-leading order} (NLO) (\emph{next-to-next-to-leading order}, NNLO), and so forth. 
...
Various textbooks, for example \ccite{Peskin:1995ev}, provide detailed discussions on the details of the renormalisation procedure.
% From Arnold:
...
\Cref{fig:xsec} shows the $pp$ collision cross sections for various processes, displaying the huge span of values over many orders of magnitudes. 
...
A full simulated event needs to consider all aspects of a $pp$ collision that are summarized in \cref{subsec:pp-collision-overview}: The matrix element, the non-perturbative regime in particular the modelling of hadronisation and showering process, and radiation. In addition, the detector response and geometry needs to be simulated, as well as other nuisances at hadron colliders such as the underlying event or multiple $pp$ collisions that simultaneously produce detector signals.
The following provides more details on the simulated MC events and the different aspects the simulation needs to cover.
...
% Dandoy
% A good MC simulation will cover all aspects of particle evolution detailed in Section 2.1, including matrix element calculation, hadronisation and showering of final states, and simulation of detector geometry and response.
...
\emph{Parton showering} occurs because there is a finite quantum probability for a single parton to split into two. The \emph{QCD splitting functions} \cite{Altarelli:1977zs} provide the theoretical foundation for dedicated parton-shower models which accurately describe this behaviour.
...
The partons hadronise when their energy decreases and the strong coupling becomes large. There are several phenomenological approaches for modelling this non-perturbative process, one of which is the \emph{Lund-String-Model}. 
...
\paragraph{Decays}
The particles produced in the scattering process or in the process of hadronisation are often unstable and further decay. Many of these decays, for example decaying $b$-hadrons, provide useful information about the event. %Dedicated algorithms exist to find the particles which initiate the decay chains.
...
The exact detector geometry and the detector response to different particles must be simulated, which is done using the {\verb GEANT4 } toolkit \cite{Agostinelli:2002hh}. 
% using the 
