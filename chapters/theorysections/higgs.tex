...
The success story culminated in the discovery of a new particle by the ATLAS and CMS Collaborations in 2012~\cite{Aad:2012tfa,Chatrchyan:2012xdj}, as by now the experimental evidence is overwhelming that the measured particle is the long sought-after Higgs boson predicted by the SM. This year (2022) marks the ten years anniversary of the Higgs boson discovery which is celebrated with a paper \cite{NATURE-Paper} that summarizes all state-of-the art Higgs measurements.
...
This section starts with a brief motivation for conducting Higgs boson measurements. \Cref{subsec:higgschannels} then provides an overview of the different production and decay modes of the Higgs boson and discusses their experimental accessibility at the LHC (\cref{subsec:exp-accessibility}). \Cref{subsec:xsec-measurements} outlines the different type of Higgs cross-section measurements that are conducted at the LHC, before \cref{subsec:} concludes with a brief overview of the experimental status at the time of writing.
...
Furthermore, the Higgs boson is a prime candidate for searching for physics beyond the SM and thus relevant to many of the open fundamental questions (see \cref{subsec:limitations}). \cite{2019BHeinemann}
This includes, for example, dark matter models (e.g. \ccite{Baumgart_2009,Kaplan_2009,Dienes_2012}) which predict interactions of the Higgs boson with yet unknown particles that are candidates for dark matter, or investigations of the process of EWSB itself by testing two-Higgs-doublet models~\cite{Branco_2012} that predict the existence of additional particles similar to the Higgs boson.

The above highly motivates performing precision measurements of the Higgs sector to test both the predictions of the SM and theories beyond the SM at an ever greater statistical precision. 

Only with a precise knowledge of Higgs processes and an understanding of what exact role the Higgs boson plays in nature, fundamental physics gets closer to answering open fundamental questions.  


% From gianotti
% o gain even deeper insights into the Higgs boson and its role in fundamental physics

OR: Precisely measuring the properties and cross-sections of the Higgs boson is needed in order to understand the exact role it plays in nature. 
...
% provide stringent constraints on many models of new phenomena beyond the
% Standard Model.
...
Once the Higgs boson is produced, it decays almost instantaneously with lifetime of about $10^{-22}$ seconds \cite{PDG2020}.
...
\begin{figure}
  ...
  \end{center}
  \caption{(a) Higgs boson production cross-sections as a function of the LHC centre-of-mass energy. The line widths represent the respective theory uncertainties. Taken from Ref.~\cite{YR4}. (b) Branching ratios of the Higgs boson for a mass of 125\,\GeV. Values taken from Ref.~\cite{PDG2020}.}
  %  \label{fig:higgsbr}
\end{figure}
...
\subsection{Experimental sensitivity of Higgs production at the LHC}
...
The $H\rightarrow b\bar{b}$ decay accounts for 58\% of all Higgs boson decays, but the experimental sensitivity to measure events with this decay mode is limited. The reason is the purely hadronic final state of $H\rightarrow b\bar{b}$ decays, which makes it difficult to distinguish these Higgs events from the overwhelming background of QCD events at the LHC. 
...
For a process with an initial state $i$ and a final state $f$, the number of observed Higgs boson events is therefore typically expressed in terms of a \emph{signal strength} defined as 
\begin{equation}
  \label{eq:signal-strength}
  \mu_{if} = \left(\sigma_i / \sigma_i^\text{SM} \right) \times \left( \BR_f / \BR_f^\text{SM} \right),
\end{equation}
where ``SM'' denotes the corresponding prediction of the SM. 
...
They are typically performed when a new signal is searched for (as done for the Higgs discovery in 2012) or to experimentally establish different production modes in the SM.
...
Another drawback is that it is not easily possible to statistically combine different differential measurements. 
...
This design choices allow combining Higgs measurements of different decay channels as well as experiments, which enables more precise cross-section measurements.
...
The specific bins are chosen following two main principles: First, the experimental acceptance should be constant within each bin, which reduces the dependency on theoretical assumptions. Second, regions that are expected to be sensitive to physics effects beyond the SM are isolated, so that they can be separately assessed experimentally.
...
The data from \RunOne\ of the LHC already allowed for precise measurements of several properties of the Higgs boson.
The mass of the Higgs boson is measured to be
\begin{equation}
  m_H = 125.09 \pm 0.21\text{(stat.)} \pm 0.11\text{(syst.)} \GeV,
\end{equation}
by the latest combined measurement of the ATLAS and CMS experiment~\cite{HIGG-2014-14}.\footnote{Analyses performed by the ATLAS and CMS Collaboration using \RunTwo\ data measure a mass of $m_H = 124.97\pm0.28\,\GeV$~\cite{ATLAS-CONF-2017-046} and $m_H = 125.26 \pm 0.21\,\GeV$~\cite{Sirunyan_2020}, respectively.}
Measurements of the spin and CP of the Higgs boson confirm the SM predictions of a spin-parity of $J^P = 0^+$~\cite{HIGG-2013-17-witherratum,CMS-HIG-14-018}. Alternative hypotheses have been excluded at more than 99.9\% confidence level.
% such as its mass, spin, or parity are measured , and cross-section measurements in various regions of phase-space firmly establish the Higgs boson in the SM. 

The data collected at \RunTwo of the LHC allow for even more precise measurements that focus on the investigation of all Higgs boson production and decay processes, firmly establishing the Higgs boson in the SM. 
The latest combined Higgs measurements of the kinematic properties of Higgs boson production as well as the Higgs coupling to other particles performed by the ATLAS collaboration are nicely summarized in \ccite{NaturePaper}.
...
The following summarizes the main results.
...
\end{equation*}
when assuming that all production and decay processes scale with the same signal strength $\mu = \mu_{if}$ (see \cref{eq:signal-strength}).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Prod mode cross section
The cross sections of the different Higgs production modes times the branching ratio are measured for specific combinations of production and decay processes. The results are shown in \cref{fig:prod-per-channel} and shows the importance of the analysis of \HWW\ decays, as they provide some of the most precise measurements in particular for the ggF and VBF production of the Higgs. 
...
The couplings of the Higgs boson to individual particles can be measured by parametrizing the cross section times branching ratios for the individual Higgs processes in terms of coupling strengths modifiers, $\kappa$, following the $\kappa$-framework \cite{LHCHandbookV3}. 
The modifier $\kappa_p$ for a production or decay via the coupling to a given particle $p$ is defined as  $\kappa_p^2 = \sigma_p / \sigma_p^\text{SM}$ or $\kappa_p^2 = \Gamma_p/\Gamma_p^\text{SM}$, respectively, where $\Gamma_p$ is the partial decay width into a pair of particles $p$.\cite{NaturePaper}
Different statistical models with varying assumptions that are detailed in \cite{NaturePaper} may be considered. 
When assuming that there are only SM processes and independently treating coupling strength modifiers for all included particles ($W$, $Z$, $t$, $b$, $c$, $\tau$, and $\mu$) the results can be visualized as shown in \cref{fig:h-couplings}. This shows a remarkable consistency of the SM. 
...
\kappa_t$ (coloured circle markers), or $\kappa_c$ left free-floating in the fit (grey
...
Investigated for example by searching for events with Higgs decays to particles that are invisible to the detector \cite{ATLASInvisible1,Aad_2022}.
