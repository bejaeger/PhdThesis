...
\emph{fermions} with half-integer spin and \emph{bosons} with integer spin. The fermions (sometimes called \emph{matter particles}) interact with each other via exchanges of \emph{gauge bosons} (also called \emph{force carriers}) that mediate the fundamental forces. 
...
In order to describe the behaviour of a physical system, the respective equations of motions can be derived from the action,
\begin{equation}
  \label{eq:action}
...
Local gauge invariance implies, that the physical content of the theory stays the same when performing re-definitions of the particle fields,
\begin{equation}
  \psi \rightarrow e^{-iT} \psi,
\end{equation}
independently at every space-time point. Here, $T$ is the generator of a certain symmetry group and generally dependent on the space-time coordinate, $T \rightarrow T(\pmb{x})$. 
...
\label{subsec:qcd}
QCD is a non-abelian gauge theory associated to the SU(3)$_C$ symmetry. The subindex $C$ refers to the colour which is the conserved quantity under SU(3)$_C$ transformations.
...
\end{equation}
The unification is based on two major empirical findings:
...
Another mechanism is required to explain the existence of the massive gauge fields \Wpm and \Zboson.
% and $\gamma$. 
% Moreover, also the simple inclusion of fermion mass terms is forbidden, given that a term
% \begin{equation}
%   -m_f \bar{\psi} \psi = -m_f \left( \bar{\psi}_R\psi_L + \bar{\psi}_L\psi_R \right)
%   \label{eq:fermionmassterm}
% \end{equation}
% violates SU(2)$_L$ gauge invariance because $\psi_L$ transforms as a doublet and $\psi_R$ as a singlet. 
