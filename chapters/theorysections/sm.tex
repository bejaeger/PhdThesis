...
The SM is a collection of relativistic quantum field theories (QFT) that describe the interactions between all known fundamental particles. The fundamental particles (also \emph{elementary particles}) are grouped according to their quantum numbers, and their interactions are described by the requirement of local gauge invariance with respect to the non-Abelian gauge group
\begin{equation}
...
\end{figure}
\todo{Maybe put graphic on full page alone OR make it bigger}
...
\end{equation}
that is, $\phi_1 = \phi_2 = \phi_4 = 0$ and $\phi_3 = v$, which spontaneously breaks the SU(2)$_L$ $\times$ U(1)$_Y$ symmetry.
