...
Several criteria, defined in \cref{chap:objects}, are imposed on the objects to guarantee a high selection efficiency of signal events while minimizing the contamination of background events. 
...
This section also discusses how known differences between the simulated event samples and data are corrected and extensively draws from \ccite{HWWPaper}.
...
\begin{table}
    ...
    \begin{tabular}{l@{\hskip 0.5in} l}
        ...
        \bottomrule
    \end{tabular}
    \caption{List of object selection criteria, acceptances, and algorithms used in the \HWW\ analysis.}
...
The isolation working points used are about 86\% efficient for electrons with $\pT = 30\,$GeV~\cite{EGAM-2018-01} and on average about 89\% efficient for muons with $20 < \pT < 100\,$GeV~\cite{MUON-2018-03}.
...
Finally, the \METSig observable is used in the DNN employed in the VBF measurement.
...
To account for the differences introduced by, for example, reconstruction and identification algorithms, data-to-simulation \emph{scale factors} (SFs) are applied at the level of the fully reconstructed event. The SFs are derived in dedicated analyses and generally given as
\begin{equation}
...
The SFs are either applied to the full event (\emph{efficiency SF}) or only to the four-momentum of individual objects (\emph{four-momentum SF}). Only four-momentum SFs can alter the shape of distributions. %, which has important consequences for the statistical analysis.
Uncertainties on these corrections are propagated to the final statistical analysis.

Another sample correction is applied to correct for differences in the underlying pile-up conditions assumed in the simulated events and that found in data.\footnote{This is needed since the exact data taking were not known when the simulated samples were produced.}
