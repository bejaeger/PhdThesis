...
Uncertainties arising from the choice of the renormalisation ($\mu_R$) and factorisation scales ($\mu_F$) are derived by both coherent and incoherent variations of ${\mu^{\text{var}_R , \mu^{\text{var}}_F } = {v_R, v_F } × {\mu^{\text{nom}_R , \mu^{\text{nom}}_F }$, with $v_R, v_F = 0.5, 1, 2$ excluding the combinations ${v_R, v_F } = {0.5, 2}, {2, 0.5}$. 
This defines 7 different variations and the total scale uncertainty is estimated to be the maximum variation. 
%defining a 7-point 2168 envelope. The total scale uncertainty is defined by the maximum variation of the 7-point envelope.

% PDF uncertainties:
The PDF and alpha_s uncertainties are evaluated from the standard deviation of 100 NNPDF3.0NLO replicas. 
The PDF replicas and αhigh,low s are included as alternative weights in the nominal Sherpa samples.
...
NO Pow+Pythia8 vs Pow+H7 comparison for shower cause it's covered by QSF and CSSKIN
...
% Int Note: The difference between the prediction of Powheg+Pythia8 and the aMCAtNlo+Pythia8 sample is taken as a matching uncertainty.
