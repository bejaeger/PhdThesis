...
The latter is quantified by means of the expected discovery significance of the VBF signal, which was $2.6\,\sigma$ for the previous VBF analysis based on a boosted decision tree (BDT), and $6.2\,\sigma$ for the analysis presented. 
...
The analysis presented is also crucial input to combined Higgs boson measurements~\cite{NaturePaper}, where several analyses are combined that study different combinations of Higgs boson production and decay processes. These results are summarized in \cref{chap:higgs}.
...
Using the $\kappa$-framework, the coupling-strength modifier between the Higgs boson and vector bosons ($HV$, with $V = W, Z$) is measured to be $\kappa_{V} = 1.035 \pm 0.031$, assuming the same modifier for the $HW$ and $HZ$ coupling.
