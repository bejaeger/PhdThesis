...
The most relevant background contributions come from events with top quarks ($\ttbar/Wt$), non-resonant $WW$ production ($WW$), $W$ production in association with jets where a jet is misidentified as a lepton (Mis-Id), other diboson processes (Other $VV$($V$)), and mentioned Drell-Yan (\Zgamma) processes. 
% \begin{itemize}
...
The expected background composition in each of the SRs are shown in \cref{fig:bkg-composition}.
\begin{figure}
    \subfloat[ggF \ZeroJet category]{
        \newImageResizeCustom{0.45}{figures/plots/bkg-fractions/pie-chart-fractions-ggF0jet.pdf} \hfill
    } 
    \subfloat[ggF \OneJet category]{
        \newImageResizeCustom{0.45}{figures/plots/bkg-fractions/pie-chart-fractions-ggF1jet.pdf} \hfill
    } \\
    \subfloat[ggF \TwoJet category]{
        \newImageResizeCustom{0.45}{figures/plots/bkg-fractions/pie-chart-fractions-ggF2jet.pdf} \hfill
    } 
    \subfloat[VBF \TwoJet category]{
        \newImageResizeCustom{0.45}{figures/plots/bkg-fractions/pie-chart-fractions-vbf.pdf} \hfill
    } 
...
The descriptions and results presented in this chapter and published in~\cref{HWWPaper} supersede similar measurements performed with the same dataset but based on a previous version of the analysis~\cite{ATLAS-CONF-2021-014}.
