...
Events are rejected from the VBF category if they either contain an additional jet with \ptGT{30}\,GeV between the two leading jets in pseudorapity (known as \emph{central jet veto} (CJV)) or contain a lepton at a larger pseudorapidity than both of the jets (known as \emph{outside lepton veto} (OLV)).
...
In addition, events from $V(\to qq) H$ production are highly suppressed by the requirements $|\mjj  - 85| \le 15\,\GeV$ and $\Delta y_{jj} \le 1.2$. 
These selections exploit that the $V(\to qq) H$ process, in contrast to the ggF \TwoJet process, have two jets with a large rapidity gap between them and an invariant mass close to the $Z$ or the $W$ boson mass.
...
\begin{figure}[h!]
  \centering
    \includegraphics[width=0.85\textwidth]{\paperfiguredir/11-POI/STXS_CompositionMap}
