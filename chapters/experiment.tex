...
Layers of high-density \emph{passive material} stop the particles and are alternated with layers of \emph{active material} that measure the signals induced by particles.
The total measured signal is proportional to the energy of the incident particle.

\TDinote{Reword above to reflect the fact that also active layer causes energy loss}
% comment from Mike
...
Each secondary particle carries a fraction of the original energy and itself interacts with the calorimeter. This results in the formation of a \emph{calorimeter shower} with gradually decreasing the energy per particle as the shower evolves.
...
A material can be characterized by its radiation length, $X_0$, which denotes the length over which an electron or positron loses on average all but  $1/e$ of its energy via bremssstrahlung. Since the probability for interacting via bremsstrahlung is proportional to $1 / m^2$, where $m$ is the mass of the particle, muons only deposit little energy in the calorimeters and thus cannot be stopped.\footnote{While muons deposit only little energy in the calorimeters, it is important to consider these energy losses. This becomes more important as the energy of the muon increases, because the amount of energy that is lost due to bremsstrahlung increases with the energy of the muon.}
...
In the plastic scintillators, incoming photons produce light via electron-positron pair production, Compton scattering, or the photoelectric effect, depending on the energy of the photon\footnote{Pair production dominates at higher energies larger than several \MeV, Compton scattering in the \MeV-range, and the photoelectric effect at lower energies below 1\,\MeV. The exact energy regime depends on the material.}. The light is routed with wavelength shifting fibers to two photomultiplier tubes where it is converted to an electric pulse. The pulse is converted to an energy measurement by using data from dedicated runs~\cite{PERF-2007-01}.
...
The transverse component of the energy, $\ETvec = \ET \hat{n}$, or momentum, $\pTvec = \pT \hat{n}$, of a particle is determined by the projection of the total energy or momentum onto the $x$-$y$ plane.
...
The calorimeter signals provide fast information, so that they are used for trigger purposes, which is described further below.
...
The simulations produce outputs of identical format as the actual experimental data so that the same software can be used for further processing of the events.
