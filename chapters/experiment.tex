...
The ATLAS experiment~\cite{PERF-2007-01} is one of the two multipurpose detectors measuring the particle-particle collisions produced by the \emph{Large Hadron Collider} (LHC)~\cite{Evans:2008zzb} to an extreme precision. It is designed to measure a broad range of physics processes with a specific focus on observing and measuring the Higgs boson.
...
\FloatBarrier
\input{tables/experiment/lhc-runs}
...
This section first provides an overview of the detection principles used to measure the products of the $pp$ collision events and gives detailed information on the detector setup thereafter.

\subsection{Detection principles}
\label{subsec:measurement-principles}
Two types of measurements can be broadly distinguished: the reconstruction of trajectories of charged particles which is known as \emph{tracking}, and the measurement of the energy of particles via total absorption in blocks of instrumented material, a principle known as \emph{calorimetry}. 

\subsubsection{Tracking}
%To reconstruct the trajectories of charged particles in the ATLAS detector, 
Tracks from charged particles can be reconstructed by measuring discrete space points and recognizing their patterns. 
Space point measurements are provided by detecting the signals (or \emph{hits}) left by traversing charged particle in finely segmented detector layers with known position. Different techniques are used in the ATLAS detector for this purpose:
\paragraph{Silicon semiconductor sensors} detect electron-hole pairs that are induced in a p-n junction when a charged particle traverses the material. They can be segmented very finely and thus deliver precise spatial measurements. 
...
\newline
To enable momentum measurements of charged particles, the tracking devices are immersed in magnetic fields that force the particles on a curved path. Their momenta can be determined from the radius of their reconstructed tracks. The resolution of the momentum measurement deteriorates linearly with the particle momentum. 
...
Layers of highly-dense \emph{passive material} stop the particles and are alternated with layers of \emph{active material} that measure the signals induced by the particles.
%The total measured signal is proportional to the energy of the incident particle.
...
As an incident particle interacts with the passive layers through different mechanisms, a cascade of secondary particles is formed.
Each secondary particle carries a fraction of the original energy and itself interacts with the calorimeter. This results in the formation of a \emph{calorimeter shower} with gradually decreasing energy per particle as the shower evolves. 
...
Electromagnetic showers are initiated by electrons, positrons, and photons and are dominated by interactions through \emph{bremsstrahlung} and electron-positron \emph{pair-production}. 
...
A useful characteristic of a material is its radiation length, $X_0$, which denotes the length over which an electron or positron loses on average all but  $1/e$ of its energy by bremssstrahlung. Since the probability for interacting via bremsstrahlung is proportional to $1 / m^2$, where $m$ is the mass of the particle, muons cannot be stopped by the calorimeter.

Hadronic showers are more complex due to the rich nature of hadronic interactions. An incident hadron initiates their showers by inelastic hadronic interactions with nuclei. This results in the production of several new particles that are mostly mesons.
...
More details on the characteristics of hadronic showers and the implications for calorimeter measurements are left to \cref{chap:calibration}. 
...
The active layers in the ATLAS calorimeter measure the signals from the particles participating in the showers and consist of \emph{liquid argon} (LAr) or plastic scintillating tiles.
The LAr-based systems are embedded in cryostats and measure the current that is induced in the liquid by charged particles. 
...
In the plastic scintillators, the incoming photons produce light via electron-positron pair production, Compton scattering, or photoelectric effect, depending on the energy of the photon\footnote{Pair production dominates at higher energies larger than several \MeV, Compton scattering in the \MeV-range, and the photoelectric effect at lower energies below 1\,\MeV. The exact energy regime depends on the material.}. The light is then collected and converted to an electric pulse at photomultiplier tubes. 
...
The \emph{inner detector} (ID) is the innermost layer and serves as a tracking device for charged-particle trajectories and is surrounded by a large \emph{solenoid magnet}.
A large \emph{calorimeter system} is placed outside the solenoid and stops almost all electromagnetic and hadronically interacting particles to provide energy measurements. 
...
\begin{figure}
    ...
    Together they form the inner detector and are surrounded by a large \emph{solenoid magnet}.
    The calorimeter system is placed outside the solenoid and includes the \emph{LAr electromagnetic calorimeters}, that targets the measurement of electromagnetic showers, as well as the \emph{tile calorimeters}, \emph{LAr hadronic end-caps}, and the \emph{LAr forward calorimeters}, that measure and contain hadronic activity.
...
The ID reconstructed tracks from charged-particle hits and provides information about the particle positions as close as \SI{27.5}{\milli\meter} to the interaction point.
The ID tracks are also used for momentum measurements, impact parameter determination, as well as primary and secondary vertexing.
...
Three silicon \emph{pixels} in the barrel and each end-cap typically provide four space points for each charged particle. 
Four barrel layers (nine disk layers) of silicon-microstrip \emph{semiconductor trackers} (SCT) in the barrel (each end-cap) are composed of a double layer of strips arranged at an angle of \SI{40}{\milli\radian} relative to each other to provide additional space points. Four double layers are crossed by each track.
...
The \emph{transition radaition tracker} TRT consists of a large number of straw tubes filled with xenon-based gas that provide on average 36 additional hits. The TRT covers a range \absetaST{2.0} and provides information in only the $R$-$\phi$ plane with a resolution of \SI{130}{\micro\meter}.
...
The ATLAS sampling calorimeter measures and contains electromagnetic as well as hadronic showers and its primary task is to measure the energy of particles. 
It is also sensitive to the position and direction of incident particles, as it is segmented into different cells in the $\eta-\phi$ plane and into several layers in the longitudinal direction.
The segmentation also allows quantifying other characteristics of the showers, which is used for particle identification.
The signal collection is very fast so that the calorimeters are also used for trigger purposes, which is described further below.
...
Depending on the \abseta region, the \emph{LAr electromagnetic calorimeter} (ECAL) and hadronic calorimeters -- consisting of the \emph{tile calorimeter}, \emph{LAr hadronic end-caps} (HEC), and the \emph{LAr forward calorimeters} (Fcal) -- provide a stopping power of $22-38\,X_0$ and $7-10\,\lambda$, respectively.
This limits the level of punch-through to the muon system almost entirely to the irreducible level of muons and neutrinos. 
...
\FloatBarrier
\input{tables/experiment/calorimeter-characteristics}
...
% Previously more precise but too much numbers
...
In the transition region between the barrel and end-caps (\absetaBT{1.37}{1.52}), also known as ``crack'', a lot of material amounting to several $X_0$ lies in front of the calorimeter, which renders photon and electron identification difficult.
...
To account for the energy that particles loose upstream of the ECAL, when traversing the ID and solenoid magnet, a presampler consisting of a thin LAr layer is placed in front of the ECAL in the range \absetaST{1.8}.
...
Their granularity is much coarser than that of the ECAL which is shown in \cref{tab:ATLAScalorimeter-parameters}.
...
The HEC provides a coverage of \absetaBT{1.5}{3.2} and is segmented in two layers in depth. They consist of one end-cap wheel on each side of the detector.
...
\begin{figure}
    ...
    \caption[Schematic of the ATLAS calorimeters showing the different subcomponents.]{Schematic of the ATLAS calorimeters showing the different subcomponents. Details on the different subcomponents are provided in the text. Taken from \ccite{PERF-2007-01}.}
...
Therefore, the main purpose of the large MS is to measure muon-track hits for muon reconstruction as well as to provide fast information of a potential muon candidate to the trigger system.
\Cref{fig:ATLASmuonspectrometer} shows the MS including the large superconducting toroid magnets that are placed in between several gaseous muon chambers.
The 8 barrel toroid magnets and 16 end-cap magnets cover the range \absetaST{1.4} and \absetaBT{1.6}{2.7}, respectively. They provide magnetic bending power of \numrange{0.5}{1}\,T to measure the muons' momenta. Precision-tracking chambers operate with an argon-based mixture and are arranged in three layers in both the barrel and end-cap regions. They cover a range \absetaST{2.7} and mostly consist of \emph{Monitored drift tubes}, that provide six to eight $\eta$ measurements. Only the innermost layer in at \absetaBT{2}{2.7} is made out of multiwire proportional chambers called \emph{Cathode strip chambers} that are more resistant against radiation damage. They provide four space points in the $\eta$-$\phi$ plane. The detector chambers with fast readouts consist of \emph{Resistive-plate chambers} (RPCs) in the central-region within \absetaST{1.05} and \emph{Thin-gap chambers} (TGCs) in the end-cap regions at \absetaBT{1.05}{2.4}. 
Their main purpose is to provide fast information to the L1 trigger system (discussed below in \cref{sec:trigger-system}), but also to provide information complementary to the precision trackers to allow for a three-dimensional track reconstruction of the muon candidates.
...
The hardware-based \emph{Level-1} (L1) trigger reduces the rate of collision events from \SI{40}{\mega\hertz} to \SI{100}{\kilo\hertz} and provides inputs to the software-based \emph{High-level trigger} (HLT), which makes further selections to reduce the rate to around \SI{1000}{\hertz}.
...
The simulations produce outputs of the same format as the actual experiment so that the same software can be used for further processing of the events.
...
The recorded data is further filtered before being considered for physics analyses.
