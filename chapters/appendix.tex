...
This nicely shows the kinematics and topologies the DNN selects to define regions pure in the VBF signal.
    ...
    \begin{landscape}
        ...
        \begin{table}
            ...
            \caption[Expected number of events at different selection stages.]{Expected number of events at different selection stages to define the (a) VBF \TwoJet SRs and CRs and (b) the ggF \TwoJet SRs and CRs. For (a) the selections from the row labelled ``Preselection'' to the row labelled ``OLV'' are applied subsequently; for (b) the selections from the row labelled ``Preselection'' to the row labelled ``fail OLV $||$ fail CJV'' are applied subsequently. All respective other rows include all selections that define the indicated regions as described in \cref{sec:event-categorization}.}
    ...
    Excellent agreement between the data and the SM predictions is observed.
    \begin{figure}[htp]
        \centering
        \subfloat[]{
        ...
        \caption[Post-fit $\mT$ distributions in the ggF $WW$ control regions.]{
            ...
            The red arrows in the lower panel of (a) indicate that the central value of the data lies above or below the window. The last bin of each distribution is inclusive (includes the overflow).
    ...
    First, the separation in four regions based on the two-dimensional discriminant as mentioned above is also applied to the CRs. One top-quark and one \Ztautau CR is used in each of the four regions and one $qq \to WW$ CR is used in the ggF-sensitive region with $0 < \text{DNN}_\mathrm{out}^\mathrm{VBF} < 0.1$. No $qq \to WW$ CR can be defined for the other regions due to limited statistical power.
...
The small improvement of the VBF measurement is in part due to a slightly finer binning used in the fits for the VBF-sensitive region, but is also a consequence of the more precise measurement of \muGGF.
